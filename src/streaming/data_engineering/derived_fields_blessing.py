...
def compute_discount_amount(total_price: float) -> float:
    """Apply a 10% discount for orders over $100."""
    if total_price > 100:
        return round(total_price * 0.10, 2)
    return 0.0
...
def enrich_message(
    ...
    discount_amount = compute_discount_amount(total_price)
...
def get_tax_rate(region_id: str, region_lookup: dict[str, float]) -> float:
    ...
    Arguments:
        region_id: The region identifier from the message.
        region_lookup: A dict mapping region_id to tax_rate_pct.
